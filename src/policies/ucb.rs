    ...
    A: Clone + Eq + Hash,
{
    ...
    fn select(&self, arms: &IndexSet<A>, _rng: &mut dyn rand::RngCore) -> Option<A> {
        if arms.is_empty() {
            return None;
        }
...
mod tests {
    ...
    fn test_ucb_balances_exploration_exploitation() {
        ...
        policy.update(&[1, 1, 1, 2, 2, 3], &[0.5, 0.4, 0.6, 0.9, 0.8, 0.7]);
    ...
    fn test_ucb_deterministic_selection() {
        ...
        let choice1 = Policy::select(&policy, &arms, &mut rng1 as &mut dyn rand::RngCore).unwrap();
        let choice2 = Policy::select(&policy, &arms, &mut rng2 as &mut dyn rand::RngCore).unwrap();
